...
- See the potential package in an R script.
- Key differences between packages and scripts.
- Define package build time.
- Avoid side effects.
...
- A good first step is to refactor this code. 
    ...
    - [Jenny Bryan's Code Smells and Feels](https://github.com/jennybc/code-smells-and-feels)
    - [Emily Riederer's RMarkdown Driven Development (RmdDD)](https://emilyriederer.netlify.app/post/rmarkdown-driven-development/)
    - [Emily Riederer's RMarkdown Driven Development: the Technical Appendix](https://emilyriederer.netlify.app/post/rmddd-tech-appendix/)
...
- [Attaching](https://r-pkgs.org/dependencies-mindset-background.html#sec-dependencies-attach-vs-load) a package does not put the functions in the global workspace.
...
- and re-document (here, just updating the NAMESPACE) with `devtools::document()`
...
- Package was declared incorrectly
   ...
   -  [`usethis::use_package`](https://r-pkgs.org/whole-game.html#use_package)
...
**Note:** To fix our initial problem with loading a CSV file, we've created a dataframe `lookup_table`. However, Chapter 7 Data provides more guidance and recommendations on how to load datasets properly.
...
- (`utils::globalVariables` example)[https://github.com/tidyverse/dbplyr/blob/7edf5d607fd6b0b897721ea96d1c9ca9401f0f9b/R/backend-redshift.R#L144]
- (Vignette: Using dplyr in packages)[https://dplyr.tidyverse.org/articles/in-packages.html]
- (A note on notes)[https://r-pkgs.org/release.html#double-r-cmd-checking]
...
**Source of the problem:** The `now <- Sys.time()` function lies outside the `outfile_path` definition, so it is executed when the package is built, but never again. *Code outside your functions is only built once at build time* (when the package code is compiled). 
...
See [withr](https://withr.r-lib.org) for more information
